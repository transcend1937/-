...
DATA_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "assets/railway_data.json")
...
.bureau-card .card-header .name {
    font-size: 16px;
    font-weight: 600;
}
.bureau-card .card-header .total {
    font-size: 18px;
    font-weight: 700;
    color: #2563eb;
}
.bureau-card .card-header .total small {
    font-size: 12px;
    font-weight: 400;
    color: #6b7280;
}
.bureau-card .card-header .arrow {
    color: #9ca3af;
    font-size: 18px;
    margin-left: 8px;
}
...
<div class="notice">
    📢 温馨提示：所有招录数据均来自中国铁路人才招聘网的官方公示
</div>

<div class="tabs">
    <div class="tab active" onclick="switchTab('bureau')">
        🏢 按路局 <span class="badge" id="bureauCount">0</span>
    </div>
    <div class="tab" onclick="switchTab('major')">
        📚 按专业 <span class="badge" id="majorCount">0</span>
    </div>
</div>

<div class="search-bar">
    <input type="text" id="searchInput" placeholder="🔍 搜索路局或专业名称..." oninput="filterData()">
</div>
...
async function init() {
    try {
        const resp = await fetch('/railway/api/data');
        ...
        document.getElementById('summaryBar').textContent = 
            '📊 共 ' + data.total_bureaus + ' 个路局 · ' + data.total_majors + ' 个专业 · ' + data.grand_total + ' 人招录数据';
        renderBureau();
    } catch(e) {
...
function switchTab(tab) {
    ...
    document.querySelectorAll('.tab')[tab === 'bureau' ? 0 : 1].classList.add('active');
...
function renderBureau(query) {
    ...
    if (query) {
        bureaus = bureaus.filter(function(b) {
            return b.name.includes(query) || b.majors.some(function(m) { return m.name.includes(query); });
        });
    }
    
    // Stats
    const total = bureaus.reduce(function(s, b) { return s + b.total.total; }, 0);
    const top = bureaus.length > 0 ? bureaus[0].name + ' ' + bureaus[0].total.total + '人' : '-';
    ...
    bureaus.forEach(function(b, i) {
        const pct = b.total.total > 0 ? Math.round(b.total.male / b.total.total * 100) : 0;
        html += '<div class="bureau-card" onclick="toggleBureau(' + i + ')">' +
            '<div class="card-header">' +
            '<span class="name">🚉 ' + b.name + '</span>' +
            '<span><span class="total">' + b.total.total + '<small>人</small></span>' +
            '<span class="arrow">▾</span></span>' +
            '</div>' +
            '<div class="card-detail">' +
            '<div style="display:flex;gap:8px;margin-bottom:6px;font-size:12px;color:#6b7280">' +
            '<span>👨 男' + b.total.male + ' (' + b.total.male_pct + ')</span>' +
            '<span>👩 女' + b.total.female + ' (' + b.total.female_pct + ')</span>' +
            '<span>📚 ' + b.majors.length + '个专业</span>' +
            '</div>';
        b.majors.forEach(function(m) {
...
function toggleBureau(idx) {
    const cards = document.querySelectorAll('.bureau-card');
...
function renderMajor(query) {
    ...
    if (query) {
        majors = majors.filter(function(m) { return m.name.includes(query); });
    }
    
    const totalMale = majors.reduce(function(s, m) { return s + m.male; }, 0);
    const totalFemale = majors.reduce(function(s, m) { return s + m.female; }, 0);
    const totalAll = majors.reduce(function(s, m) { return s + m.total; }, 0);
    const topMajor = majors.length > 0 ? majors[0].name + ' ' + majors[0].total + '人' : '-';
    ...
    document.getElementById('statsRow').innerHTML =
        ...
        '<div class="stat-card"><div class="num" style="font-size:13px;line-height:1.3">' + topMajor + '</div><div class="label">最多</div></div>';
    
    let html = '<div class="bureau-grid">';
    majors.forEach(function(m, i) {
        const barMale = m.male > 0 && m.total > 0 ? Math.round(m.male / m.total * 100) : 0;
        html += '<div class="bureau-card" onclick="showMajorDetail(' + i + ')">' +
            '<div class="card-header">' +
            '<span class="name">📖 ' + m.name + '</span>' +
            '<span><span class="total">' + m.total + '<small>人</small></span>' +
            '<span class="arrow">▸</span></span>' +
            '</div>' +
            '<div class="card-detail">' +
            '<div style="display:flex;gap:8px;margin-bottom:6px;font-size:12px;color:#6b7280">' +
            '<span>👨 男' + m.male + '</span>' +
            '<span>👩 女' + m.female + '</span>' +
            '<span>🏢 ' + m.bureau_count + '个路局</span>' +
            '</div>' +
            '</div></div>';
    });
    html += '</div>';
...
function showMajorDetail(idx) {
    const m = filteredData.length > 0 ? filteredData[idx] : data.majors[idx];
    ...
    m.bureaus.forEach(function(b) {
        html += '<span class="bureau-chip">' + b + '</span>';
    });
    html += '</div>';
